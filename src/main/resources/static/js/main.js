;(function () {
	...
	var sendCalculate = function() {
		var btn = $(document.querySelector("#calculator #btn"));

		btn.click(function(){
			var form = this.form;

			var getType = function (radioNodeList) {
				for (var i = 0, len = radioNodeList.length; i < len; i++) {
					var el = radioNodeList[i];
					if (el.checked) {
						return el.value;
					}
				}
			...
			if ( form.elements.credit_sum.value ) {
				var summa = form.elements.credit_sum.value,
					type = getType(form.elements.payment_type),
